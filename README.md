...
| [synapse-time-series](#synapse-time-series) | 시계열 데이터 임포트 (ULG → dm_schema) | 1.0.0 | data |
...
시계열 어노테이터 데이터 임포트 플러그인. PX4 ULG 등 raw 시계열 데이터를 dm_schema JSON으로 변환합니다.
...
| `/synapse-time-series:create-track-config` | 고객 요구 기반 트랙 설정 YAML 생성 |
...
| **synapse-schema-debugger** | dm_schema 검증 오류 자동 진단 및 수정 제안 |
