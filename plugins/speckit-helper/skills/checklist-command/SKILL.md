...
description: "Run the `speckit-helper:checklist` workflow from the original Claude slash command. Use when the user asks to generate quality checklists that validate specification completeness and clarity"
---

<!-- Generated by tools/generate-agent-marketplaces.py; edit the source command or agent-plugin.yaml instead. -->

# speckit-helper:checklist Command Workflow
...
Extract the feature-slug and optional `--domain` flag from `the user request or explicit arguments`.
...
- If `the user request or explicit arguments` is empty, use Glob to list directories under `.speckit/`:
