---
name: refine-command
description: "Run the `speckit-helper:refine` workflow from the original Claude slash command. Use when the user asks to update an existing specification with new information or changed requirements"
---

<!-- Generated by tools/generate-agent-marketplaces.py; edit the source command or agent-plugin.yaml instead. -->

# speckit-helper:refine Command Workflow
...
Extract the feature-slug and change description from `the user request or explicit arguments`:
...
- If `the user request or explicit arguments` is empty, print an error:
...
- If ambiguities are found in the new additions, use **ask the user a concise clarification** to
