...
description: "Run the `speckit-helper:implement` workflow from the original Claude slash command. Use when the user asks to execute implementation from task list with dependency-aware ordering"
---

<!-- Generated by tools/generate-agent-marketplaces.py; edit the source command or agent-plugin.yaml instead. -->

# speckit-helper:implement Command Workflow
...
Extract targeting flags from `the user request or explicit arguments`:
