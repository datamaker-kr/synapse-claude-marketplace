...
description: "Run the `speckit-helper:clarify` workflow from the original Claude slash command. Use when the user asks to clarify ambiguous areas in specifications through targeted Q&A"
---

<!-- Generated by tools/generate-agent-marketplaces.py; edit the source command or agent-plugin.yaml instead. -->

# speckit-helper:clarify Command Workflow
...
- **If a feature-slug is provided in `the user request or explicit arguments`:** Use it directly. Verify that
...
- **If `the user request or explicit arguments` is empty:** Find the most recently modified spec by scanning
...
Use the **ask the user a concise clarification** tool to present questions to the user and collect
