...
description: "Run the `speckit-helper:analyze` workflow from the original Claude slash command. Use when the user asks to analyze specifications for consistency, gaps, and quality issues (read-only)"
---
...
- If `the user request or explicit arguments` is non-empty, use the first positional argument as the feature-slug.
