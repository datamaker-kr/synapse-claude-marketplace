...
description: "Run the `speckit-helper:specify` workflow from the original Claude slash command. Use when the user asks to create a feature specification from natural language requirements"
---

<!-- Generated by tools/generate-agent-marketplaces.py; edit the source command or agent-plugin.yaml instead. -->

# speckit-helper:specify Command Workflow
...
Read the feature description from `the user request or explicit arguments`.

- If `the user request or explicit arguments` is non-empty, use it as the feature description.
- If `the user request or explicit arguments` is empty, stop and ask the user:
...
If `the user request or explicit arguments` is empty and the user does not provide a description when prompted:
