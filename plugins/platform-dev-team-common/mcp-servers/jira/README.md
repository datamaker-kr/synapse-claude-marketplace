...
```
CHANGELOG.md ──> changelog_extract_tickets ──> 티켓 ID 목록
                                                    │
Git branches ──> changelog_check_branches  ──> 브랜치 포함 정보
                                                    │
                                              jira-sync 스킬
                                           (상태 전이 규칙 적용)
                                                    │
                                    ┌───────────────┼───────────────┐
                              jira_transition   jira_update_field   SKIP
                              (상태 변경)       (customfield 변경)
```
...
| Command (`commands/sync-jira.md`) | `/sync-jira` 커맨드 오케스트레이션 절차 |
...
```bash
/sync-jira                          # unreleased 섹션 동기화 (기본)
/sync-jira --section unreleased     # 명시적으로 unreleased 섹션 지정
/sync-jira --section v2026.1.1      # 특정 릴리스 섹션만 동기화
/sync-jira --dry-run                # 변경 사항 미리보기 (실제 변경 없음)
```
...
```
0단계: 프로젝트 확인 (CWD, Git 루트, CHANGELOG.md 존재 여부)
  ↓
1단계: changelog_extract_tickets → 티켓 ID 추출
  ↓
2단계: changelog_check_branches → 브랜치 포함 여부 확인 (git fetch 후)
  ↓
3단계: jira_get_ticket → 각 티켓의 현재 Jira 상태 조회
  ↓
4단계: 상태 전이 규칙 적용 (아래 규칙표 참조)
  ↓
5단계: 결과 리포트 출력
```
...
MCP 서버가 제공하는 도구들은 `/sync-jira` 외에도 Claude와의 대화에서 직접 사용할 수 있습니다.
...
commands/sync-jira.md         # /sync-jira 커맨드 오케스트레이션 절차
```
