---
name: upload-command
description: "Run the `synapse-upload:upload` workflow from the original Claude slash command. Use when the user asks to upload files from a local or remote source to a Synapse data collection"
---

<!-- Generated by tools/generate-agent-marketplaces.py; edit the source command or agent-plugin.yaml instead. -->

# synapse-upload:upload Command Workflow
...
Parse any arguments provided. For **every missing required parameter**, use `ask the user a concise clarification` to ask the user. Walk through them in a logical order:
