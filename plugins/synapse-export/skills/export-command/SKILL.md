---
name: export-command
description: "Run the `synapse-export:export` workflow from the original Claude slash command. Use when the user asks to export annotations, ground truth, or task data from a Synapse project"
---

<!-- Generated by tools/generate-agent-marketplaces.py; edit the source command or agent-plugin.yaml instead. -->

# synapse-export:export Command Workflow
...
**IMPORTANT**: This command is designed to be fully interactive. Users may invoke it with no arguments at all — just `/synapse-export:export`. When this happens, guide them step-by-step through the entire process using `ask the user a concise clarification`. Never fail or dump usage text when arguments are missing.
...
For **every missing parameter**, use `ask the user a concise clarification` to ask the user.
