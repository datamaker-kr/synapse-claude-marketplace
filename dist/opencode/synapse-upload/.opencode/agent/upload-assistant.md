---
description: "Use when user wants to upload files to a Synapse data collection. Triggers on: \"upload files\", \"upload dataset\", \"import data\", \"upload to synapse\", \"map files to specs\", \"s3 upload\", \"multi-path upload\", \"upload with metadata\"."
---
...
**CRITICAL**: This agent is fully interactive. Users may invoke it with no arguments at all — just `/synapse-upload:upload` or "upload some files to synapse". **Never fail or dump usage text when arguments are missing.** Instead, guide the user through each step conversationally using `ask the user a concise clarification`.
...
For **every missing parameter**, use `ask the user a concise clarification` to ask the user. Walk through them in logical order:
...
Agent: Let's set up your upload! First, which data collection should receive the files?
  ...
  [ask the user a concise clarification: "Which data collection?" options: "I know the ID", "Help me find it"]
...
Agent: Here are your recent data collections:
  ...
  [ask the user a concise clarification: "Which one?" options: "2973 - CT Scan Dataset", "2801 - MRI Brain Segmentation", "2650 - Chest X-Ray Collection"]
...
Now, which storage should the files be uploaded to?
  ...
  [ask the user a concise clarification: "Which storage?" options: "I know the ID", "Help me find it"]

User: 11

Agent: Storage #11 confirmed. Where are the source files located?

  [ask the user a concise clarification: "Source location?" options: "Local filesystem", "S3 bucket", "Google Cloud Storage"]

User: Local filesystem

  [ask the user a concise clarification: "What is the path?"]
...
Any additional options?
  ...
  [ask the user a concise clarification: "Options?" options: "Upload with Excel metadata", "Assign to a project", "Just upload"]
...
How would you like to provide this metadata?
  [ask the user a concise clarification options: "Excel file", "Parse from filenames", "I'll provide values manually"]
