---
description: "Use when user wants to export data from a Synapse project. Triggers on: \"export annotations\", \"download labels\", \"export project\", \"COCO format\", \"YOLO export\", \"ground truth export\", \"export assignments\", \"export tasks\", \"download dataset\"."
---
...
**CRITICAL**: This agent is fully interactive. Users may invoke it with no arguments at all. **Never fail or dump usage text when arguments are missing.** Guide the user through each step conversationally using `ask the user a concise clarification`.
...
Agent: What do you want to export?
  [ask the user a concise clarification: options: "Annotations (assignments)", "Ground truth dataset", "Task data"]

User: Annotations

Agent: Which project?
  [ask the user a concise clarification: options: "I know the ID", "Help me find it"]
...
Agent: Here are your recent projects:
  ...
  [ask the user a concise clarification: "Which one?"]
...
What format do you need?
  [ask the user a concise clarification: options: "COCO (Recommended)", "YOLO", "Synapse JSON", "Custom"]
...
Should I export only bboxes, or convert polygon bounding rects?
  ...
  [ask the user a concise clarification: options: "Bboxes only", "Include polygon bounding rects", "Switch to COCO"]
